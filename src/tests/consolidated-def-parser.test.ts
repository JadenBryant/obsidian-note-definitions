...
test("Keys are stored as lowercase of words", () => {
	...
	expect(
		definitions.find((def) => def.word === "Multiple-word definition")?.key,
...
test("Definitions are parsed correctly", () => {
	...
	expect(
		definitions.find((def) => def.key === "alias definition")?.definition,
	).toBe("This tests if the alias function works.");
	expect(
		definitions.find((def) => def.key === "markdown support")?.definition,
	).toBe("Markdown syntax _should_ *work*.");
});

test("Positions are parsed correctly", () => {
	expect(definitions.find((def) => def.key === "first")?.position?.from).toBe(
		0,
	);
	expect(definitions.find((def) => def.key === "first")?.position?.to).toBe(
		4,
	);
	expect(
		definitions.find((def) => def.key === "multiple-word definition")
	...
	expect(
		definitions.find((def) => def.key === "alias definition")?.position?.to,
	).toBe(18);
	expect(
	...
	expect(
		definitions.find((def) => def.key === "markdown support")?.position?.to,
	).toBe(22);
});
